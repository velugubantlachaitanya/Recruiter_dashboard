...
1. **[0:00–0:30]** Intro — explain the problem: recruiters waste hours; your agent automates it
2. **[0:30–1:30]** Paste a sample JD → show AI parsing output (skills, location, experience extracted)
3. **[1:30–2:30]** Show candidate pool → ML matching running → ranked list appears
4. **[2:30–3:30]** Click on top candidate → show engagement simulation (email sent, reply detected)
5. **[3:30–4:30]** Show final shortlist with star ratings → explain Match + Interest scores
6. **[4:30–5:00]** Close with architecture diagram + mention open-source tools used

---

## 📊 Architecture Diagram (draw.io / Excalidraw)

Create a diagram at **excalidraw.com** with these boxes connected by arrows:

```
[Recruiter] → [JD Input UI]
                    ↓
            [JD Parser - Claude API]
                    ↓
         [Structured JD Requirements]
                    ↓
     [Candidate Pool] → [ML Matcher (sentence-transformers)]
                    ↓
           [Match Score + Breakdown]
                    ↓
       [Engagement Agent - Claude API]
           ↓               ↓
    [Email Service]   [AI Interview Sim]
                    ↓
           [Interest Score]
                    ↓
      [Ranking Engine (Match 60% + Interest 40%)]
                    ↓
    [⭐ Star-Rated Shortlist → Recruiter Dashboard]
```

Export as PNG and save to `architecture.png`.

---

## ✅ Submission Checklist

- [ ] Working prototype deployed (Vercel + Railway/Render)
- [ ] Public GitHub repo with README
- [ ] `instruction.md` included in repo
- [ ] `architecture.png` diagram included
- [ ] Sample input: `sample_inputs/sample_jd.txt`
- [ ] Sample output: `sample_outputs/shortlist_output.json`
- [ ] 3–5 min demo video (upload to YouTube/Loom, add link to README)
- [ ] Repo access granted to: hackathon@deccan.ai
- [ ] GitHub org access: https://github.com/hackathon-deccan-ai
- [ ] Submitted before **Monday, April 27 at 1:00 AM IST**

---

## 💡 Pro Tips for Hackathon Success

1. **Mock data first** — don't spend time scraping real profiles; create 50 rich JSON candidates
2. **Demo the happy path** — pick 2–3 candidates that score well and show the full pipeline clearly
3. **Explainability = win** — always show WHY a candidate scored the way they did
4. **Star ratings are visual** — make the UI show clear ⭐ stars, not just numbers
5. **Keep the video tight** — judges watch many; get to the demo by 30 seconds
6. **Email Chaitanya's repo link** to hackathon@deccan.ai before the deadline
